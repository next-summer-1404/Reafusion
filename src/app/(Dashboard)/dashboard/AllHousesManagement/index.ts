'use server'
import { EditAllHouseAdmin } from "@/core/Apis/Dashboard/EditAllHouseAdmin";

const EditAllHousesAction = async (state: { message: string}, formData: FormData) => {
  ...
  try {
    const response = await EditAllHouseAdmin(updatedHouseData, id);
    if (response) {
      return {
...
export default EditAllHousesAction;